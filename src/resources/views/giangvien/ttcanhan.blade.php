      ...
      <div class="row">
        <div class="col-6">
          ...
          <table id="tttable" class="table" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
            <tr><th>Mã giảng viên: </th><td>{{$tt->ma_giang_vien}}</td></tr>
  ...
  function suaTT(){
    ...
    $.ajax({
      ...
      success: function(data){ 
        if(dataGiangVien.ngaySinh != ""){
          var ngaySinh = formatDateFromString(dataGiangVien.ngaySinh);
        } else{
          var ngaySinh = '';
        } 

        $('#ctgioi').html(dataGiangVien.gioi);
        $('#ctngaySinh').html(ngaySinh);
        $('#ctdiaChi').html(dataGiangVien.diaChi);
        $('#ctsdt').html(dataGiangVien.sdt);
        $('#ctemail').html(dataGiangVien.email);

        $('#btnGroup').html(`
          <a href="#" class="btn btn-primary py-1 px-2 mr-2" style="font-size: 14px" id="btnSuaTTCaNhan"
            onclick="formSua('${dataGiangVien.gioi}', '${data.ngay_sinh}', '${dataGiangVien.diaChi}', '${dataGiangVien.sdt}', '${dataGiangVien.email}')">
            <i class="fas fa-pen mr-1"></i> Cập nhật thông tin
          </a>
          <a href="#" class="btn btn-primary py-1 px-2" style="font-size: 14px" data-toggle="modal" data-target="#modalDoiMK">
            <i class="fas fa-lock mr-1"></i> Đổi mật khẩu
